...
#define EXTRA_PRINT 0
      ...
      TEST(unit_perceptMesquite, quad_1)
      {
        EXCEPTWATCH;
      ...
      TEST(unit_perceptMesquite, quad_2)
      {
        EXCEPTWATCH;
        stk::ParallelMachine pm = MPI_COMM_WORLD ;
        MPI_Barrier( MPI_COMM_WORLD );
      ...
      TEST(unit_perceptMesquite, quad_3)
      {
        EXCEPTWATCH;
        stk::ParallelMachine pm = MPI_COMM_WORLD ;
        MPI_Barrier( MPI_COMM_WORLD );
        ...
        //if (p_size == 1 || p_size == 2)
          if (p_size <= par_size_max)
          {
...

            {
              ...
              if (err) {std::cout << err << endl;  EXPECT_TRUE(false);}
            }
      ...
      TEST(unit_perceptMesquite, quad_4)
      {
        EXCEPTWATCH;
        stk::ParallelMachine pm = MPI_COMM_WORLD ;
        MPI_Barrier( MPI_COMM_WORLD );
      ...
      TEST(unit_perceptMesquite, quad_4_1)
      {
        EXCEPTWATCH;
        stk::ParallelMachine pm = MPI_COMM_WORLD ;
        MPI_Barrier( MPI_COMM_WORLD );
      ...
      TEST(unit_perceptMesquite, hex_1)
      {
        EXCEPTWATCH;
        stk::ParallelMachine pm = MPI_COMM_WORLD ;
        MPI_Barrier( MPI_COMM_WORLD );
      ...
      Mesquite::MeshImpl *create_mesquite_mesh(PerceptMesh *eMesh, stk::mesh::Selector *boundarySelector)
      {
        ...
        const stk::mesh::BucketVector & node_buckets = eMesh->get_bulk_data()->buckets( eMesh->node_rank() );
              ...
              for (unsigned ientity = 0; ientity < num_entity_in_bucket; ientity++)
                {
                  ...
                  ++num_elem;
                  const mesh::PairIterRelation elem_nodes = element.relations( stk::topology::NODE_RANK );
        ...
        const CellTopologyData *const topology = m_eMesh.get_cell_topology(*buckets[0]);
        switch(topology->key) 
          {
      ...
      TEST(unit_perceptMesquite, hex_2)
      {
        EXCEPTWATCH;
        stk::ParallelMachine pm = MPI_COMM_WORLD ;
        MPI_Barrier( MPI_COMM_WORLD );
      ...
      TEST(unit_perceptMesquite, hex_4)
      {
        EXCEPTWATCH;
        stk::ParallelMachine pm = MPI_COMM_WORLD ;
        MPI_Barrier( MPI_COMM_WORLD );
...
#define MSQ_TEST 0
      ...
      TEST(unit_perceptMesquite, msq_hex_2)
      {
...
#define VTK_3D_DIR "./"
        ...
        if (err) {cerr << err << endl;  EXPECT_TRUE(false);}
