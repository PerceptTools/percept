...
#ifndef REDS
TEST(function, stringFunction_xy_basic)
{
  EXCEPTWATCH;
...
TEST(function, stringFunction_xy_basic_1)
{
  EXCEPTWATCH;
  ...
  EXPECT_DOUBLE_EQ(vx, x);
  double vy = eval(x, y, z, t, sfy);
  EXPECT_DOUBLE_EQ(vy, y);
  double vxy = eval(x, y, z, t, sfxy);
  EXPECT_DOUBLE_EQ(vxy, xy);
...
TEST(function, stringFunction_xy_basic_2)
{
  EXCEPTWATCH;
...
#define TEST_SF_NAMED_ARG 0
  ...
  EXPECT_EQ(sftest_domain.rank(), 1);
  EXPECT_EQ(sftest_domain.dimension(0), 3);
  EXPECT_EQ(sftest_codomain.rank(), 2);
  EXPECT_EQ(sftest_codomain.dimension(0), 2);
  EXPECT_EQ(sftest_codomain.dimension(1), 3);

  StringFunction sfx("x",Name("sfx"));
  EXPECT_EQ(sfx.getDomainDimensions().size(), 1u);
  EXPECT_EQ(sfx.getDomainDimensions()[0], 3);
  EXPECT_EQ(sfx.getCodomainDimensions().size(), 1u);
  EXPECT_EQ(sfx.getCodomainDimensions()[0], 1);
...
TEST(function, stringFunction_test_alias)
{
  EXCEPTWATCH;
...
TEST(function, stringFunction_vector_valued)
{
  EXCEPTWATCH;
  double x = 1.234;
  double y = 5.678;
...
{                                                                       \
  ...
  EXPECT_DOUBLE_EQ(vxy ## name , xy ## name);                           \
  double vxy1 ## name          = eval(x, y, z, t, sfxy1 ## name);       \
  EXPECT_DOUBLE_EQ(vxy1 ## name, xy ## name);                           \
}
...
TEST(function, stringFunction_arithmetic_ops)
{
  EXCEPTWATCH;
...
TEST(function, stringFunction_derivative)
{
  EXCEPTWATCH;
  for (unsigned ipts = 0; ipts < NPTS; ipts++)
  {
    ...
    // the two different functions should give the same result
    EXPECT_DOUBLE_EQ(dvxy, dvxy1);

    // and they should give the same result as C++
    EXPECT_DOUBLE_EQ(dvxy, -1.0);
...
TEST(function, stringFunction_derivative_1)
{
  EXCEPTWATCH;
  for (unsigned ipts = 0; ipts < NPTS; ipts++)
  {
    ...
    {
      EXPECT_DOUBLE_EQ(dvxy(ii), dvxy1(ii));
      EXPECT_DOUBLE_EQ(dvxy(ii), dvxy2(ii));
      EXPECT_DOUBLE_EQ_APPROX(dvxy(ii), dvxy_fd(ii));
    }

    // and they should give the same result as C++
    EXPECT_DOUBLE_EQ(dvxy(0), 1.0);
    EXPECT_DOUBLE_EQ(dvxy(1), -1.0);
...
TEST(function, stringFunction_derivative_2)
{
  EXCEPTWATCH;
  for (unsigned ipts = 0; ipts < NPTS; ipts++)
  {
    ...
    for (int ii = 0; ii < 3; ii++)
    {
      ...
      EXPECT_DOUBLE_EQ_APPROX_TOL(dv(ii), dv_fd(ii), 1.e-4);
    }
...
TEST(function, stringFunction_multiplePoints)
{
  EXCEPTWATCH;
  ...
  for (unsigned ipts = 0; ipts < NPTS; ipts++)
  {
    EXPECT_DOUBLE_EQ(output(ipts, 0), output_expect(ipts, 0));
  }
  /// indirection
...
//=============================================================================
//=============================================================================
TEST(function, stringFunction_expressions)
{
  EXCEPTWATCH;
  /*
...
#define DO_SF_TEST(expr)                                                \
  { using namespace std;                                                \
    ...
    EXPECT_DOUBLE_EQ_APPROX(v_loc, ve_loc);                                    \
                                                                        \
  }
...
#define DO_SF_TEST_IPT(expr,ipt)                        \
...

  {
    ...
    DO_SF_TEST(EXPR_TO_TEST1);
    DO_SF_TEST(EXPR_TO_TEST2);
    DO_SF_TEST(EXPR_TO_TEST3);
    DO_SF_TEST(EXPR_TO_TEST4);
    DO_SF_TEST(EXPR_TO_TEST5);
    DO_SF_TEST(EXPR_TO_TEST6);
  }
...
TEST(function, stringFunction_timing)
{
  EXCEPTWATCH;
