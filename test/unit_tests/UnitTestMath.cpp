...
static void test_eigen(double A[3][3], double eig_expected[3])
{
  ...
  EXPECT_DOUBLE_EQ_APPROX_TOL(eig[0], eig_expected[0], tol);
  EXPECT_DOUBLE_EQ_APPROX_TOL(eig[1], eig_expected[1], tol);
  EXPECT_DOUBLE_EQ_APPROX_TOL(eig[2], eig_expected[2], tol);
#endif
}

/// Code generated from Mathematica notebook @see eigen3x3.1.nb 
TEST(unit_math, eigen)
{
  EXCEPTWATCH;
