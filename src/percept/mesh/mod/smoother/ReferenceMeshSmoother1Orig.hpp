...
// just keeping around for history
#if 0
...
#define PRINT_2(a) do {  std::cout << "P[" << m_eMesh->get_rank() <<"] " << a << std::endl; } while(0)
  ...
  namespace percept {
    ...
    void ReferenceMeshSmoother1::get_gradient()
    {
      ...
      stk::mesh::FieldBase *cg_h_field    = (m_use_hessian_scaling? eMesh->get_field(stk::topology::NODE_RANK, "cg_h") : 0);
                  ...
                  for (unsigned i_element = 0; i_element < num_elements_in_bucket; i_element++)
                    {
                      ...
                      if ((test_analytic_grad || use_analytic_grad) && m_stage == 1)
                        {
                          //virtual double grad_metric(stk::mesh::Entity element, bool& valid, double grad[8][4]) { return 0.0; }
                          bool gmvalid = true;
      ...
      {
        stk::all_reduce( m_eMesh->get_bulk_data()->parallel() , stk::ReduceMax<1>( & m_scale ) );
        //!! FIXME
    ...
    void ReferenceMeshSmoother1::get_scale()
    {
...

      {
        ...
        stk::all_reduce( m_eMesh->get_bulk_data()->parallel() , stk::ReduceMax<1>( & m_grad_norm_scaled ) );
      ...
      if (0)
        {
          ...
          int spatialDim = m_eMesh->get_spatial_dim();
          stk::mesh::FieldBase *cg_d_field    = m_eMesh->get_field(stk::topology::NODE_RANK, "cg_d");
          stk::mesh::FieldBase *cg_g_field    = m_eMesh->get_field(stk::topology::NODE_RANK, "cg_g");
          stk::mesh::FieldBase *cg_r_field    = m_eMesh->get_field(stk::topology::NODE_RANK, "cg_r");
          ...
          for (unsigned i_node=0; i_node < idl; i_node++)
            {
              ...
              stk::mesh::Entity node_p = m_eMesh->get_bulk_data()->get_entity(stk::topology::NODE_RANK, i_node+1);
