                                 ...
                                 ErrorFieldType * error_field_no_ft, ErrorFieldType * error_field)
{
  ...
  private:
    ...
    void create_marker(std::string type, const YAML::Node& node, stk::mesh::Selector *wedge_selector)
    {
      ...
      m_marker_info->errorIndicator_                        = m_eMesh.get_fem_meta_data()->get_field<ErrorFieldType>(m_eMesh.element_rank(), m_error_indicator_field);
      ...
      m_marker_info->refineField_                           = m_eMesh.get_fem_meta_data()->get_field<RefineFieldType>(m_eMesh.element_rank(), "refine_field");
      VERIFY_OP_ON(m_marker_info->refineField_, !=, 0, "couldn't find refine field - check name and types");

      m_marker_info->refineFieldOrig_                       = m_eMesh.get_fem_meta_data()->get_field<RefineFieldType>(m_eMesh.element_rank(), "refine_field_orig");
      VERIFY_OP_ON(m_marker_info->refineFieldOrig_, !=, 0, "couldn't find refine field - check name and types");

      m_marker_info->refineLevelField_                      = m_eMesh.get_fem_meta_data()->get_field<RefineLevelType>(m_eMesh.element_rank(), "refine_level");
      ...
      if (m_eMesh.get_spatial_dim() == 3) {
        m_marker_info->transitionElementField_                = m_eMesh.get_fem_meta_data()->get_field<TransitionElementType>(m_eMesh.element_rank(), "transition_element_3");
        VERIFY_OP_ON(m_marker_info->transitionElementField_, !=, 0, "couldn't find transition element field - check name and types");
        m_marker_info->transitionElementField2d_          = m_eMesh.get_fem_meta_data()->get_field<TransitionElementType>(m_eMesh.face_rank(), "transition_element");
        VERIFY_OP_ON(m_marker_info->transitionElementField2d_, !=, 0, "couldn't find transition element field 2d - check name and types");
      }
      else {
        m_marker_info->transitionElementField_                = m_eMesh.get_fem_meta_data()->get_field<TransitionElementType>(m_eMesh.element_rank(), "transition_element");
        VERIFY_OP_ON(m_marker_info->transitionElementField_, !=, 0, "couldn't find transition element field - check name and types");
      ...
      if (m_bounding_region_type=="cylinder") {
        m_marker_info->boundingRegion_ =
          Teuchos::rcp(new CylinderBoundingRegion(m_eMesh.get_fem_meta_data()->get_field<CoordinatesFieldType >(stk::topology::NODE_RANK, "coordinates"),
                                                  m_bounding_region_radius, m_bounding_region_start,
                                                  m_bounding_region_end));
      }
      else if (m_bounding_region_type=="sphere") {
        m_marker_info->boundingRegion_ =
          Teuchos::rcp(new SphereBoundingRegion(m_eMesh.get_fem_meta_data()->get_field<CoordinatesFieldType >(stk::topology::NODE_RANK, "coordinates"),
                                                m_bounding_region_radius, m_bounding_region_center));
      }
      else if (m_bounding_region_type=="box") {
        m_marker_info->boundingRegion_ =
        Teuchos::rcp(new BoxBoundingRegion(m_eMesh.get_fem_meta_data()->get_field<CoordinatesFieldType >(stk::topology::NODE_RANK, "coordinates"),
